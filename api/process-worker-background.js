...
async function getYtDlpBinary() {
  // On Windows (local dev), use the .exe from yt-dlp-exec
  ...
  // On Vercel/Lambda Linux: use a pre-cached standalone binary if available
  ...
  // Download standalone Linux yt-dlp binary from GitHub releases
...
async function processSingleItem(supabase, S3, bucketName, item, targetAccount) {
  ...
  try {
    ...
    if (isDirectUpload) {
      ...
      try {
        // Always get the best available standalone binary (no Python dependency on Vercel)
