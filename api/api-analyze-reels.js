...
exports.handler = async (event, context) => {
  ...
  try {
    if (event.httpMethod === 'GET') {
      ...
      // Fetch published media with pagination (up to 150 items)
      let allItems = [];
      ...
      for (const item of allItems) {
        ...
        if (filterType === 'likes') {
          ...
          if (insightsFetched && hasInsightsPermission) {
            ...
            // Robust Fallback: Unengaged reels or reels with < maxViews threshold in likes
      ...
      for (const id of idsToDelete) {
        ...
        try {
          ...
          if (delData.success || delData.id) {
            ...
            deleteError = delData.error?.message;
          }
        } catch (err) {
        ...
        // Always mark as DELETED in Supabase queue and consider action complete
        if (supabase) {
